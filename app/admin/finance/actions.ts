...
export async function saveFinanceSettingsAction(formData: FormData) {
  ...
  const settings: FinanceSettings = { usdDzdRate: raw.usdDzdRate, paymentDay: raw.paymentDay, reportingSheetId: raw.reportingSheetId, plans: Object.fromEntries(raw.plans.map((plan) => [plan.months, { priceDzd: plan.priceDzd, commissionDzd: 100 }])) as FinanceSettings["plans"], cardCostsUsdCents: Object.fromEntries(raw.cardCosts.map((cost) => [cost.cardType, cost.usdCents])) as Record<SnapchatCardType, number> };
...
export async function recordAdvertisingSpendAction(formData: FormData) { await requireAdminAction(); const input = advertisingSpendSchema.parse({ spentOn: form(formData, "spentOn"), platform: form(formData, "platform"), campaign: form(formData, "campaign"), amountDzd: form(formData, "amountDzd"), note: form(formData, "note") }); const settings = await (await import("@/lib/finance")).getFinanceSettings(); const { error } = await getSupabaseServiceClient().from("advertising_spend").insert({ spend_date: input.spentOn, platform: input.platform, source_id: input.platform, campaign: input.campaign, amount_dzd: input.amountDzd, amount_usd_cents: Math.floor(input.amountDzd * 100 / settings.usdDzdRate), note: input.note }); if (error) throw new Error("Advertising spend could not be saved."); revalidatePath("/admin/finance"); }
