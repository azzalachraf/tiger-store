...
export default async function AdminClientSheetPage({
  params,
}: {
  ...
  const [summary, rows] = await Promise.all([
    getAdminFinanceSummary(adminId),
    getAdminClientSheet(adminId),
  ]);
  const name = admin.first_name || admin.username || `Admin ${adminId}`;

  return (
    <AdminShell
      title={`📋 ${name}'s client sheet`}
      description="Private internal record of this administrator's completed Telegram sales. Customer details stay in the secure admin panel."
    >
      <div className="grid gap-5">
        <div className="flex flex-wrap items-center justify-between gap-3 rounded-2xl border border-white/10 bg-white/[0.045] p-4">
          <div className="flex min-w-0 items-center gap-3">
            <span className="inline-flex h-11 w-11 shrink-0 items-center justify-center rounded-xl bg-tiger-ember/15 text-tiger-gold">
              <UserRound className="h-5 w-5" />
            </span>
            <div>
              <p className="font-black text-white">{name}</p>
              <p className="text-sm text-white/50">
                {admin.username ? `@${admin.username}` : "No Telegram username"}
              </p>
            </div>
          </div>
          <Link
            href="/admin/team"
            className="inline-flex min-h-11 items-center gap-2 rounded-xl border border-white/15 px-4 text-sm font-black text-white hover:bg-white/10"
          >
        ...
        <section className="grid gap-3 sm:grid-cols-2 xl:grid-cols-5">
          ...
          <Metric
            ...
            value={money(summary.remainingDzd)}
            accent
          />
        </section>
        ...
        <section className="overflow-hidden rounded-2xl border border-white/10 bg-white/[0.045]">
          <div className="flex items-center gap-2 border-b border-white/10 p-4">
            <ClipboardList className="h-5 w-5 text-tiger-gold" />
            <div>
              <h2 className="font-black text-white">
                Clients and completed orders
              </h2>
              <p className="mt-1 text-sm text-white/55">
                Only completed sales assigned to this administrator appear here.
              </p>
            </div>
          </div>
          {rows.length ? (
            <>
                ...
                {rows.map((row) => (
                  ...
                  >
                    <div className="flex items-start justify-between gap-3">
                      <div className="min-w-0">
                        ...
                        <p className="mt-1 text-xs font-semibold text-white/45">
                          {row.productName} · {row.planMonths} month
                          {row.planMonths === 1 ? "" : "s"}
                        </p>
                      </div>
                      <span className="shrink-0 rounded-full bg-emerald-400/10 px-2 py-1 text-xs font-black text-emerald-300">
                        {row.status}
                      </span>
                    </div>
                    <div className="mt-3 grid gap-2 text-sm text-white/65">
                      <p className="flex gap-2">
                        <Phone className="mt-0.5 h-4 w-4 shrink-0 text-tiger-gold" />
                        <span dir="ltr">{row.phone}</span>
                      </p>
                      <p className="flex min-w-0 gap-2">
                        <Mail className="mt-0.5 h-4 w-4 shrink-0 text-tiger-gold" />
                    ...
                    <div className="mt-3 flex justify-between border-t border-white/10 pt-3 text-sm">
                      ...
                      <span className="font-black text-tiger-gold">
                        {money(row.revenueDzd)}
                      </span>
                    </div>
                  </article>
                ))}
              ...
              <div className="hidden overflow-x-auto md:block">
                <table className="min-w-full text-left text-sm">
                  <thead className="bg-black/25 text-xs uppercase tracking-wide text-white/50">
                    <tr>
                  ...
                  <tbody>
                    {rows.map((row) => (
                      ...
                      >
                        ...
                        <td className="max-w-52 truncate px-4 py-4" dir="ltr">
                          {row.email}
                        </td>
                        <td className="px-4 py-4 whitespace-nowrap">
                          {dateTime(row.completedAt)}
                        </td>
                        <td className="px-4 py-4 text-right font-black text-tiger-gold">
                          {money(row.revenueDzd)}
                        </td>
                        <td className="px-4 py-4 text-right font-black">
                          {money(row.commissionDzd)}
                        </td>
                      </tr>
                    ))}
                  </tbody>
            ...
            <p className="p-8 text-center text-sm font-semibold text-white/55">
              No completed Telegram sales are assigned to this administrator
              yet.
            </p>
          )}
        </section>
...
function Metric({
  ...
  return (
    <article className="rounded-2xl border border-white/10 bg-white/[0.045] p-4">
      ...
      <p
        className={`mt-2 text-xl font-black ${accent ? "text-tiger-gold" : "text-white"}`}
      >
        {value}
      </p>
    </article>
