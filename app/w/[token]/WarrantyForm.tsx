...
type Copy = { review: string; name: string; user: string; platform: string; phone: string; paymentMethod: string; confirm: string; back: string; next: string; platformOptions: { instagram: string; snapchat: string; facebook: string; placeholder: string }; paymentOptions: { placeholder: string; baridiMob: string; binance: string; redotPay: string; flexy: string } };

export function WarrantyForm({ token, copy, action }: { token: string; copy: Copy; action: (formData: FormData) => void | Promise<void> }) {
  const [review, setReview] = useState(false);
  const [values, setValues] = useState({ name: "", username: "", platform: "", phone: "", paymentMethod: "" });
  const reviewFields: [keyof typeof values, string][] = [["name", copy.name], ["username", copy.user], ["platform", copy.platform], ["phone", copy.phone], ["paymentMethod", copy.paymentMethod]];
  ...
  return <form className="grid gap-4" onSubmit={(event) => { event.preventDefault(); if (requiredValuesComplete) setReview(true); }}>
    ...
    <label className="grid gap-1 font-bold text-[var(--text)]">{copy.platform} *<select required aria-required="true" value={values.platform} onChange={(event) => setValues({ ...values, platform: event.target.value })} className="min-h-12 rounded-xl border border-[var(--border-color)] bg-[var(--page)] px-3"><option value="" disabled>{copy.platformOptions.placeholder}</option><option value="Instagram">Instagram</option><option value="Snapchat">Snapchat</option><option value="Facebook">Facebook</option></select></label>
...
function RequiredInput({ label, type, value, minLength, maxLength, autoComplete, onChange }: { label: string; type: "text" | "tel"; value: string; minLength?: number; maxLength: number; autoComplete: string; onChange: (value: string) => void }) {
