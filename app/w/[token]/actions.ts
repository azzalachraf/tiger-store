...
export async function submitTelegramWarrantyAction(formData: FormData) { const token = String(formData.get("token") ?? ""); const firstName = String(formData.get("name") ?? "").trim(); const familyName = String(formData.get("familyName") ?? "").trim(); const input = telegramWarrantyFormSchema.parse({ name: `${firstName} ${familyName}`, username: formData.get("username"), platform: "Snapchat", phone: formData.get("phone"), email: formData.get("email"), paymentMethod: formData.get("paymentMethod") }); const phone = normalizeAlgerianPhone(input.phone); if (!phone) throw new Error("Invalid phone number."); await submitTelegramWarranty(token, { ...input, phone }); redirect(`/w/${token}`); }
