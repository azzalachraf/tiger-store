...
export function CheckoutView({ products, directProductSlug, directOption, settings, initialItems, lockedProductLink = false, allowedPaymentMethods = standardPaymentMethods }: CheckoutViewProps) {
  ...
  if (step === "complete") {
    ...
    const instagramCopy = ar
      ? { title: "تابع طلبك على Instagram", body: "أرسل صورة وصل الدفع ورمز الطلب في رسالة على Instagram لمتابعة التفعيل.", button: "إرسال الوصل على Instagram" }
      : { title: "Continue on Instagram", body: "Send your payment receipt and order code in an Instagram message to continue activation.", button: "Send receipt on Instagram" };
    return <main className="store-shell min-h-screen px-4 py-10 sm:px-6"><div className="mx-auto max-w-3xl" dir={ar ? "rtl" : "ltr"}><section className="overflow-hidden rounded-3xl border border-[var(--border-color)] bg-[var(--surface)]"><div className="border-b border-[var(--border-color)] bg-[var(--page)] p-6 text-center sm:p-9"><span className="mx-auto inline-flex h-14 w-14 items-center justify-center rounded-full bg-[#E8F7ED] text-[#16803C]"><CheckCircle2 className="h-7 w-7" /></span><p className="mt-4 text-sm font-black text-[#16803C]">{completeCopy.badge}</p><h1 className="mt-2 text-3xl font-black tracking-tight text-[var(--text)] sm:text-4xl">{completeCopy.title}</h1><p className="mx-auto mt-3 max-w-xl leading-7 text-[var(--muted-text)]">{completeCopy.body}</p></div><div className="grid gap-4 p-5 sm:grid-cols-[0.9fr_1.1fr] sm:p-7"><div className="rounded-2xl border border-[var(--border-color)] bg-[var(--page)] p-5"><ReceiptText className="h-5 w-5 text-[#C54E00]" /><p className="mt-4 text-sm font-bold text-[var(--muted-text)]">{completeCopy.code}</p><p className="mt-1 break-all text-xl font-black text-[var(--text)]" dir="ltr">{orderCode}</p><p className="mt-4 rounded-xl bg-[var(--surface)] p-3 text-sm font-bold text-[var(--text)]">{copy.status}</p></div><div><h2 className="text-lg font-black text-[var(--text)]">{completeCopy.next}</h2><ol className="mt-4 grid gap-3">{completeCopy.steps.map((item, index) => <li key={item} className="flex items-center gap-3 text-sm font-semibold text-[var(--muted-text)]"><span className="inline-flex h-7 w-7 shrink-0 items-center justify-center rounded-full bg-[#FFF1E6] text-xs font-black text-[#C54E00]">{index + 1}</span>{item}</li>)}</ol></div></div>{settings.instagramUrl ? <div className="mx-5 mb-5 rounded-2xl border border-[#FF7300]/30 bg-[#FFF1E6] p-4 sm:mx-7 sm:mb-7"><h2 className="font-black text-[#17120F]">{instagramCopy.title}</h2><p className="mt-2 text-sm font-semibold leading-6 text-[#5D4B40]">{instagramCopy.body}</p><a href={settings.instagramUrl} target="_blank" rel="noreferrer" className="mt-4 inline-flex min-h-11 items-center rounded-xl bg-[#FF7300] px-4 text-sm font-black text-[#17120F]">{instagramCopy.button}</a></div> : null}<div className="flex flex-col gap-3 border-t border-[var(--border-color)] p-5 sm:flex-row sm:justify-end sm:p-7"><Button asChild variant="secondary" className="min-h-11"><Link href="/contact"><MessageCircle className="h-4 w-4" />{completeCopy.support}</Link></Button><Button asChild className="min-h-11"><Link href="/"><Home className="h-4 w-4" />{completeCopy.home}</Link></Button></div></section></div></main>;
  }

  return <main className="store-shell min-h-screen px-3 py-6 sm:px-5 lg:px-8" dir={ar ? "rtl" : "ltr"}><div className="mx-auto max-w-3xl"><form onSubmit={sendReceipt} className="rounded-2xl border border-[var(--border-color)] bg-[var(--surface)] p-5 sm:p-7"><h1 className="text-2xl font-black text-[var(--text)]">{ar ? "إتمام الطلب" : "Complete order"}</h1><div className="mt-5 grid gap-4"><Field label={copy.name} value={name} onChange={setName} required /><Field label={copy.phone} value={phone} onChange={setPhone} required type="tel" placeholder="0550 123 456" /><Field label={copy.notes} value={notes} onChange={setNotes} textarea /><div className={`grid gap-2 ${allowedPaymentMethods.length === 4 ? "grid-cols-2 sm:grid-cols-4" : "grid-cols-3"}`}>{allowedPaymentMethods.map((method) => <button key={method} type="button" onClick={() => setPaymentMethod(method)} className={`min-h-12 rounded-xl border px-2 text-sm font-black ${paymentMethod === method ? "border-[#FF7300] bg-[#FF7300] text-[#17120F]" : "border-[var(--border-color)] bg-[var(--page)] text-[var(--text)]"}`}>{method}</button>)}</div><p className="rounded-xl border border-[var(--border-color)] bg-[var(--page)] p-4 text-sm font-bold text-[var(--text)]">{copy.paymentDetails}: <span dir="ltr">{instructions}</span></p><label className="grid cursor-pointer gap-2 text-sm font-bold text-[var(--text)]"><span>{copy.chooseReceipt}</span><span className="flex min-h-14 items-center gap-3 rounded-xl border border-[#FF7300]/60 px-3"><Upload className="h-7 w-7 shrink-0 fill-none text-[#FF7300]" aria-hidden="true" /><span className="min-w-0 flex-1 truncate text-[var(--muted-text)]">{receipt?.name || (ar ? "لم يتم اختيار صورة" : "No image selected")}</span><span className="shrink-0 rounded-lg bg-[#FF7300] px-3 py-2 font-black text-[#17120F]">{ar ? "اختيار صورة" : "Choose image"}</span><input type="file" accept="image/png,image/jpeg,image/webp" required onChange={(event) => setReceipt(event.target.files?.[0] ?? null)} className="sr-only" /></span></label>{error && <p role="alert" className="text-sm font-bold text-[#C62828]">{error}</p>}<Button type="submit" disabled={saving} className="min-h-12"><Upload className="h-4 w-4" />{saving ? (ar ? "جارٍ الإرسال..." : "Sending...") : copy.upload}</Button></div></form>
      <aside className="mt-4 rounded-2xl border border-[var(--border-color)] bg-[var(--surface)] p-5"><h2 className="text-xl font-black text-[var(--text)]">{copy.summary}</h2><div className="mt-4 grid gap-3">{items.map((item) => <div key={item.id} className="rounded-xl bg-[var(--page)] p-3"><p className="font-black text-[var(--text)]">{ar ? item.nameAr : item.name}</p><p className="mt-1 text-xs font-bold text-[var(--muted-text)]">{ar ? item.optionAr : item.option} ×{item.quantity}</p><p className="mt-2 font-black text-[#C54E00]">{formatPriceDZD(item.price * item.quantity, locale)}</p></div>)}</div><div className="mt-4 flex justify-between border-t border-[var(--border-color)] pt-4 font-black text-[var(--text)]"><span>{copy.total}</span><span className="text-[#C54E00]">{formatPriceDZD(total, locale)}</span></div><Button asChild variant="ghost" className="mt-4 w-full"><Link href="/shop">{copy.back}</Link></Button></aside>
    </div></main>;
}

function Field({ label, value, onChange, required, type = "text", textarea, placeholder }: { label: string; value: string; onChange: (value: string) => void; required?: boolean; type?: string; textarea?: boolean; placeholder?: string }) { return <label className="grid gap-2 text-sm font-bold text-[var(--text)]">{label}{textarea ? <textarea rows={2} value={value} onChange={(event) => onChange(event.target.value)} className="min-h-20 resize-y rounded-xl border border-[var(--border-color)] bg-[var(--page)] px-4 py-3 text-base text-[var(--text)]" /> : <input value={value} onChange={(event) => onChange(event.target.value)} required={required} type={type} placeholder={placeholder} className="min-h-12 rounded-xl border border-[var(--border-color)] bg-[var(--page)] px-4 text-base text-[var(--text)]" />}</label>; }
