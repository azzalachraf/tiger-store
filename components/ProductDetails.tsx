...
export function ProductDetails({ product }: { product: Product }) {
  ...
  const info = [
    ["Duration", selected?.duration], ["Device compatibility", selected?.compatibilityEn || details?.compatibilityEn], ["Activation type", details?.activationMethodEn || product.activationTypeEn], ["Activation time", "15min - 12h after payment"], ["Warranty", details?.warrantyEn], ["Account type", details?.accountTypeEn], ["Included credits", details?.creditsEn], ["Storage", details?.storageEn],
  ...
  return <section className="space-y-8" dir="ltr">
    ...
    <div className="grid gap-6 lg:grid-cols-2 lg:items-start">
      <div className="overflow-hidden rounded-xl border border-[#DDD7CE] bg-[#F3F0EA]"><div className="relative aspect-[4/5]"><Image src={product.image} alt={`${product.name} product artwork`} fill priority sizes="(min-width: 1024px) 50vw, 100vw" className="object-contain p-3" /></div></div>
      <aside className="rounded-xl border border-[#DDD7CE] bg-[#F3F0EA] p-5 text-[#151515] sm:p-7">
    ...
    <section className="rounded-xl border border-[#DDD7CE] bg-[#F3F0EA] p-5 sm:p-7"><h2 className="text-2xl font-black text-[#151515]">Product details</h2><ul className="mt-5 grid gap-3 sm:grid-cols-2">{product.featuresEn.map((feature) => <li key={feature} className="flex items-center gap-3 text-sm font-bold leading-6 text-[#151515]"><CheckCircle2 className="h-4 w-4 shrink-0 text-[#C54E00]" />{feature}</li>)}</ul></section>
    {product.faqs?.length ? <section className="rounded-xl border border-[#DDD7CE] bg-[#F3F0EA] p-5 sm:p-7"><h2 className="text-2xl font-black text-[#151515]">Frequently asked questions</h2><div className="mt-5 grid gap-3">{product.faqs.map((faq) => <details key={faq.questionEn} className="border-b border-[#DDD7CE] pb-4"><summary className="cursor-pointer font-black text-[#151515]">{faq.questionEn}</summary><p className="mt-3 text-sm font-semibold leading-7 text-[#66615B]">{faq.answerEn}</p></details>)}</div></section> : null}
  </section>;
}
