...
export function ProductDetails({ product }: { product: Product }) {
  ...
  return <section className="min-w-0 space-y-6 sm:space-y-8" dir={locale === "ar" ? "rtl" : "ltr"}>
    ...
    <section className="rounded-xl border border-[var(--border-color)] bg-[var(--surface)] p-5 sm:p-7"><h2 className="text-2xl font-black text-[var(--text)]">{t(locale, "productDetails")}</h2><ul className="mt-5 flex gap-3 overflow-x-auto pb-2">{(locale === "ar" ? product.featuresAr : product.featuresEn).map((feature) => <li key={feature} className="flex min-h-12 shrink-0 items-center gap-3 rounded-full border border-[var(--border-color)] bg-[var(--page)] px-5 text-lg font-black leading-6 text-[var(--text)]"><CheckCircle2 className="h-5 w-5 shrink-0 text-[#C54E00]" />{translateSupport(feature, locale)}</li>)}</ul></section>
