...
export function ProductDetails({ product }: { product: Product }) {
  ...
  const info = [
    [t(locale, "duration"), selected?.duration], [t(locale, "compatibility"), selected?.compatibilityEn || details?.compatibilityEn], [t(locale, "activationType"), details?.activationMethodEn || product.activationTypeEn], [t(locale, "activationTime"), t(locale, "activationMessage")], [t(locale, "warranty"), details?.warrantyEn], ["Account type", details?.accountTypeEn], ["Included credits", details?.creditsEn], ["Storage", details?.storageEn],
  ...
  return <section className="space-y-8" dir={locale === "ar" ? "rtl" : "ltr"}>
    ...
    <div className="grid gap-6 lg:grid-cols-2 lg:items-start">
      <div className="overflow-hidden rounded-xl border border-[var(--border-color)] bg-[var(--surface)]"><div className="relative aspect-[4/5]"><Image src={product.image} alt={`${product.name} product artwork`} fill priority sizes="(min-width: 1024px) 50vw, 100vw" className="object-contain p-3" /></div></div>
      <aside className="rounded-xl border border-[var(--border-color)] bg-[var(--surface)] p-5 text-[var(--text)] sm:p-7">
        ...
        {offers.length > 1 && <div className="mt-7"><h2 className="text-sm font-black uppercase tracking-[.14em] text-black/55">{t(locale, "chooseDuration")}</h2><div className="mt-3 grid gap-3">{offers.map((offer) => { const active = selectedKey === offer.key; return <button key={offer.key} type="button" disabled={offer.available === false} onClick={() => setSelectedKey(offer.key)} className={cn("flex min-h-16 items-center justify-between gap-3 rounded-md border p-4 text-left transition-colors focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-tiger-ember disabled:cursor-not-allowed disabled:opacity-45", active ? "border-tiger-ember bg-[#FFF1E6]" : "border-[#DDD7CE] bg-[#F3F0EA] hover:border-[#FF7300]")}><span className="flex items-center gap-3"><span className={cn("flex h-5 w-5 items-center justify-center rounded-full border", active ? "border-tiger-ember bg-tiger-ember text-black" : "border-black/25")}>{active && <Check className="h-3.5 w-3.5" />}</span><span><span className="block font-black">{offer.duration}</span>{offer.compatibilityEn && <span className="mt-0.5 block text-xs font-bold text-black/55">{offer.compatibilityEn}</span>}</span></span><span className="font-black text-[#d85b00]">{money(offer.price)}</span></button>; })}</div></div>}
    ...
    <section className="rounded-xl border border-[var(--border-color)] bg-[var(--surface)] p-5 sm:p-7"><h2 className="text-2xl font-black text-[var(--text)]">{t(locale, "productDetails")}</h2><ul className="mt-5 grid gap-3 sm:grid-cols-2">{product.featuresEn.map((feature) => <li key={feature} className="flex items-center gap-3 text-sm font-bold leading-6 text-[var(--text)]"><CheckCircle2 className="h-4 w-4 shrink-0 text-[#C54E00]" />{feature}</li>)}</ul></section>
    {product.faqs?.length ? <section className="rounded-xl border border-[var(--border-color)] bg-[var(--surface)] p-5 sm:p-7"><h2 className="text-2xl font-black text-[var(--text)]">Frequently asked questions</h2><div className="mt-5 grid gap-3">{product.faqs.map((faq) => <details key={faq.questionEn} className="border-b border-[var(--border-color)] pb-4"><summary className="cursor-pointer font-black text-[var(--text)]">{faq.questionEn}</summary><p className="mt-3 text-sm font-semibold leading-7 text-[var(--muted-text)]">{faq.answerEn}</p></details>)}</div></section> : null}
  </section>;
}
