...
export function ProductDetails({ product }: { product: Product }) {
  ...
  const isSnapchat = product.slug === "snapchat-plus";
  const productFeatures = isSnapchat ? (locale === "ar" ? ["تفعيل من دون كلمة مرور", "الضمان طيلة مدة الإشتراك", "التفعيل فوري"] : locale === "fr" ? ["Activation sans mot de passe", "Garantie pendant toute la durée", "Activation immédiate"] : ["Activation without a password", "Warranty for the full subscription", "Instant activation"]) : (locale === "ar" ? product.featuresAr : product.featuresEn).map((feature) => translateSupport(feature, locale));
  const actionCopy = isArabic ? { payment: "الدفع والتفعيل", paymentBody: "اختر الخطة، أكمل بيانات الطلب، ثم حوّل المبلغ وارفع وصل الدفع. يبدأ التفعيل بعد تأكيد الدفع.", policy: "الضمان والاسترجاع", policyBody: "إذا كان الخلل من طرف Tiger Store ومشمولاً بالضمان، نحاول الاستبدال أولاً. إذا تعذّر ذلك، يُحسب استرجاع الجزء غير المستعمل من الضمان بالدينار الصحيح. المشاكل الناتجة عن العميل غير مشمولة." } : locale === "fr" ? { payment: "Paiement et activation", paymentBody: "Choisissez l’offre, complétez vos informations, effectuez le transfert puis ajoutez le justificatif. L’activation commence après vérification du paiement.", policy: "Garantie et remboursement", policyBody: "Pour un problème couvert causé par Tiger Store, nous tentons d’abord un remplacement. Si cela est impossible, la période couverte non utilisée est remboursée proportionnellement en DZD entiers. Les problèmes causés par le client sont exclus." } : { payment: "Payment and activation", paymentBody: "Choose the plan, complete the order details, then transfer and upload the receipt. Activation begins after payment verification.", policy: "Warranty and refunds", policyBody: "For a Tiger Store-caused covered failure, we attempt replacement first. If impossible, the unused covered period is refunded proportionally in whole DZD. Customer-caused problems are excluded." };
  return <section className="min-w-0 space-y-6 sm:space-y-8" dir={locale === "ar" ? "rtl" : "ltr"}>
    <nav className="flex min-w-0 items-center gap-2 overflow-hidden text-xs font-bold text-[#5F6368] sm:text-sm"><Link href="/" className="shrink-0 hover:text-[#C54E00]">{t(locale, "home")}</Link><span>/</span><span className="min-w-0 truncate text-[#151515]" dir="auto">{locale === "ar" ? product.nameAr : product.name}</span></nav>
    <div className="grid min-w-0 gap-4 sm:gap-6 lg:grid-cols-2 lg:items-start">
      <div className="min-w-0 overflow-hidden rounded-xl border border-[var(--border-color)] bg-[var(--surface)]"><div className="relative aspect-square sm:aspect-[4/5]"><Image src={product.image} alt={`${locale === "ar" ? product.nameAr : product.name} — ${t(locale, "productArtwork")}`} fill priority sizes="(min-width: 1024px) 50vw, 100vw" className="object-contain p-2 sm:p-3" /></div></div>
      <aside className="min-w-0 rounded-xl border border-[var(--border-color)] bg-[var(--surface)] p-4 text-[var(--text)] sm:p-7">
        <h1 className="text-3xl font-black leading-tight sm:text-5xl" dir="auto">{locale === "ar" ? product.nameAr : product.name}</h1>
        ...
        {!isSnapchat && <><div className="mt-7 border-y border-black/10 py-5"><div className="flex flex-wrap items-end justify-between gap-3"><div><p className="text-xs font-black tracking-[.14em] text-black/55">{t(locale, "price")}</p><p className="mt-1 text-3xl font-black text-[#d85b00]">{selected && selected.price > 0 ? money(selected.price) : t(locale, "priceUnavailable")}</p></div><p className={`text-sm font-black ${unavailable ? "text-black/45" : "text-emerald-700"}`}>{unavailable ? t(locale, "outOfStock") : t(locale, "inStock")}</p></div></div><div className="mt-5 grid gap-3">{info.map(([label, content]) => <div key={label} className="flex items-start justify-between gap-5 text-sm"><span className="shrink-0 font-bold text-black/50">{label}</span><span className="text-end font-bold leading-6">{content}</span></div>)}</div>{detailValue(details, locale, "notice") && <p className="mt-5 border-s-2 border-tiger-ember ps-3 text-sm font-bold leading-6 text-black/70">{detailValue(details, locale, "notice")}</p>}</>}
        {unavailable ? <><Button disabled className="mt-7 min-h-12 w-full rounded-full">{t(locale, "outOfStock")}</Button><StockAlertForm productSlug={product.slug} optionId={selected?.id} /></> : <div className="mt-7 grid gap-2"><Button asChild className="min-h-12 rounded-full"><Link href={checkoutHref}>{t(locale, "buyNow")}</Link></Button></div>}
      </aside>
    </div>
    <section className="rounded-xl border border-[var(--border-color)] bg-[var(--surface)] p-5 sm:p-7"><h2 className="text-2xl font-black text-[var(--text)]">{t(locale, "productDetails")}</h2><ul className="mt-5 flex gap-3 overflow-x-auto pb-2">{productFeatures.map((feature) => <li key={feature} className="flex min-h-12 shrink-0 items-center gap-3 rounded-full border border-[var(--border-color)] bg-[var(--page)] px-5 text-lg font-black leading-6 text-[var(--text)]"><CheckCircle2 className="h-5 w-5 shrink-0 text-[#16803C]" />{feature}</li>)}</ul></section>
