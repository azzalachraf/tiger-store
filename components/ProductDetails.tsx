  ...
  return <section className="space-y-8" dir={locale === "ar" ? "rtl" : "ltr"}>
    ...
    <div className="grid gap-6 lg:grid-cols-2 lg:items-start">
      ...
      <aside className="rounded-xl border border-[var(--border-color)] bg-[var(--surface)] p-5 text-[var(--text)] sm:p-7">
        ...
        {offers.length > 1 && <div className="mt-7"><h2 className="text-sm font-black tracking-[.14em] text-black/55">{t(locale, "chooseDuration")}</h2><div className="mt-3 grid gap-3">{offers.map((offer) => { const active = selectedKey === offer.key; return <button key={offer.key} type="button" disabled={offer.available === false} onClick={() => setSelectedKey(offer.key)} className={cn("flex min-h-16 items-center justify-between gap-3 rounded-md border p-4 text-start transition-colors focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-tiger-ember disabled:cursor-not-allowed disabled:opacity-45", active ? "border-tiger-ember bg-[#FFF1E6]" : "border-[#DDD7CE] bg-[#F3F0EA] hover:border-[#FF7300]")}><span className="flex items-center gap-3"><span className={cn("flex h-5 w-5 items-center justify-center rounded-full border", active ? "border-tiger-ember bg-tiger-ember text-black" : "border-black/25")}>{active && <Check className="h-3.5 w-3.5" />}</span><span><span className="block font-black">{optionValue(offer, locale, "duration")}</span>{optionValue(offer, locale, "compatibility") && <span className="mt-0.5 block text-xs font-bold text-black/55">{optionValue(offer, locale, "compatibility")}</span>}</span></span><span className="font-black text-[#d85b00]" dir="ltr">{money(offer.price)}</span></button>; })}</div></div>}
