...
export function StockAlertForm({ productSlug, optionId }: { productSlug: string; optionId?: string }) {
  ...
  return <form onSubmit={submit} className="mt-4 grid gap-2 rounded-xl border border-black/10 bg-[#F3F0EA] p-3">
    ...
    <div className="flex gap-2"><input id={`stock-alert-${productSlug}`} value={phone} onChange={(event) => setPhone(event.target.value)} inputMode="tel" autoComplete="tel" placeholder="0556 97 45 93" required className="min-h-11 min-w-0 flex-1 rounded-lg border border-black/15 bg-white px-3 text-black" /><button type="submit" className="min-h-11 rounded-lg bg-[#151515] px-3 text-sm font-bold text-white">Notify me</button></div>
