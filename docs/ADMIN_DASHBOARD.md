...
- Overview, Statistics and Finances open with **Net profit** selected. **Show → Revenue** switches the result and chart labels without changing stored data. Both views use the same completed-sale ledger; cancelled/refunded orders are excluded, Telegram sales are not counted twice as website orders, and recorded zero-credit salary arrangements remain zero. Net profit deducts recorded card costs, admin credit and any recorded advertising, including advertising-only days. Missing card costs remain clearly flagged. Focused admin/plan totals retain the existing explicit before-advertising treatment.
