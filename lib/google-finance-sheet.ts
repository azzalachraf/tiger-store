...
export async function syncFinanceReporting() {
  ...
  for (const spend of reports.advertisingSpend) { const row = daily.get(String(spend.spend_date)) ?? [0, 0, 0, 0]; row[3] -= Number(spend.amount_dzd); daily.set(String(spend.spend_date), row); }
  ...
  const advertising = [["Date", "Platform", "Campaign", "Spend DZD", "Notes"], ...reports.advertisingSpend.map((spend) => [String(spend.spend_date), String(spend.platform), String(spend.campaign), Number(spend.amount_dzd), String(spend.note)])];
